...
if(ua.indexOf("ipad") != -1 || ua.indexOf("iphone") != -1 || ua.indexOf("android") != -1 || ua.indexOf("ipod") != -1 || ua.indexOf("windows ce") != -1 || ua.indexOf("windows phone") != -1 || (navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1)){
	mobile = true;
}
...
(function($) {
	if(mobile == true){
		...
		$.when(
		    $.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/mobile.js" ),
		...
		//cam and mic permission (Chrome 64+ only for HTML5 recorder)
		camPerm = "";
		micPerm = "";
		...
		noCameraTxt= "You need a camera to record video";
		...
		//recorder ID
		recorderId = "";
		if(flashvars["recorderId"]){
			recorderId = flashvars["recorderId"];
		}

		$.ajax({
			url: ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/precheck.php",
			data: {accountHash:flashvars["accountHash"], environmentId:(flashvars["eid"] ? flashvars["eid"] : '1')},
			success: function (data, status) {
				...
				if(res.client == "html5"){
					if(document.location.protocol.indexOf("https") != -1 || (window.location.protocol == "http:" && (window.location.hostname == "localhost" || window.location.hostname == "127.0.0.1" || window.location.hostname == "::1"))){
				...
				//check permission dialog state on Chrome 64+ for the HTML5 recorder
				if(webRtcClient == true){
					if(get_browser().name == "Chrome" && get_browser().version >= 64){
				...
				$("#hdfvr-content").height(size["height"]);
				...
				if(closedAccount == 0){
					...
					if(maintenance == 1){
						...
						languageFileURL = ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/"+(flashvars["lang"] ? flashvars["lang"] : 'translations/'+langCode+'.xml');
						if(flashvars["lang"]){
						...
						$.ajax({
							...
							success: function(data){
								...
								if(flashvars["sis"] == 0 || flashvars["sis"] == undefined){

									css.innerHTML = "div.pipeRecordRTC{position: relative; width:"+size["width"]+"px;height:"+size["height"]+"px;background-color:"+bgCol+";border-radius:"+cornerRadius+"px;text-align:center;} div.pipeRecordRTC span {position:absolute;margin: 0;left:50%;top:50%;margin-right: -50%;transform: translate(-50%, -50%);font-family:sans-serif;color:"+normalCol+"; font-size:18px; cursor: pointer;}  #pipeStartRecording:hover{color:"+overCol+";} #pipeRecordScreen:hover{color:"+overCol+";} #pipeStartUploading{position: absolute; left: -99999rem} .custom-upload{display:inline-block;font-family:sans-serif;color:"+normalCol+"; font-size:18px; cursor: pointer;} .pipe-upload-label{margin-bottom:0px; font-weight:normal;cursor: pointer;}  label:hover{color:"+overCol+";}  .uploadAnother{position:absolute;margin: 0;left:50%;top:57%;margin-right: -50%;transform: translate(-50%, -50%);font-family:sans-serif;color:"+normalCol+"; font-size:12px; text-decoration:underline;cursor: pointer;}";
									...
									if(showPoweredBy ==  0){
										document.getElementById('hdfvr-content').innerHTML = '<div class="pipeRecordRTC"><span><div id="pipeStartRecording" tabindex="0"><img src="'+("https:" == document.location.protocol ? "https://" : "http://")+'cdn.addpipe.com/1.3/img/webcam.png" style="vertical-align: middle;"/> '+recordTxt+' </div><div  id="pipe-upload-form"><div id="pipe-upload-wrap" tabindex="0"><img id="pipeUploadIcon" src="'+("https:" == document.location.protocol ? "https://" : "http://")+'cdn.addpipe.com/1.3/img/icon_upload.png" style="vertical-align: middle;"/><label class="pipe-upload-label" for="pipeStartUploading" class="custom-upload"> &nbsp;'+uploadRecTxt+'</label><input name="FileInput" id="pipeStartUploading" type="file" ' + accept + ' value="Start Uploading" tabindex="-1"/></div></div><div id="output" style="cursor:default;"></div><div id="pipeRecordScreen" tabindex="0"><img src="'+("https:" == document.location.protocol ? "https://" : "http://")+'cdn.addpipe.com/1.3/img/screen.png" style="vertical-align: middle;"/>&nbsp;'+recordScreenTxt+'</div></span><div id="uploadAnother" class="uploadAnother" tabindex="0">'+uploadOrRecordAnotherTxt+'</div></div>';
									}else{
										document.getElementById('hdfvr-content').innerHTML = '<div class="pipeRecordRTC"><span><div id="pipeStartRecording" tabindex="0"><img src="'+("https:" == document.location.protocol ? "https://" : "http://")+'cdn.addpipe.com/1.3/img/webcam.png" style="vertical-align: middle;"/> '+recordTxt+' </div><div  id="pipe-upload-form"><div id="pipe-upload-wrap" tabindex="0"><img id="pipeUploadIcon" src="'+("https:" == document.location.protocol ? "https://" : "http://")+'cdn.addpipe.com/1.3/img/icon_upload.png" style="vertical-align: middle;"/><label class="pipe-upload-label" for="pipeStartUploading" class="custom-upload"> &nbsp;'+uploadRecTxt+'</label><input name="FileInput" id="pipeStartUploading" type="file" ' + accept + ' value="Start Uploading" tabindex="-1"/></div></div><div id="output" style="cursor:default;"></div><div id="pipeRecordScreen" tabindex="0"><img src="'+("https:" == document.location.protocol ? "https://" : "http://")+'cdn.addpipe.com/1.3/img/screen.png" style="vertical-align: middle;"/>&nbsp;'+recordScreenTxt+'</div></span><div id="uploadAnother" class="uploadAnother" tabindex="0">'+uploadOrRecordAnotherTxt+'</div><p id="pipeClickPowered" title="Pipe Video Recorder" style="position:absolute;bottom: 0px; left:'+(size["width"]/2 - 40)+'px; text-align:center;text-decoration:underline;font-family:sans-serif;font-size:10px; color:#334455;cursor: pointer;" tabindex="0">Powered by Pipe</p></div>';
										$("#pipeClickPowered").click(poweredByPipe);
										$('#pipeClickPowered').keydown(function(event){
										  if (event.which == 13) {
									...
									$("#uploadAnother").hide();
									$("#pipe-upload-wrap").hide();
									if(flashvars["dup"] == 1 && (accType == 50 || accType == 1)){
										$("#pipe-upload-wrap").show();
									}

									document.getElementById("pipeUploadIcon").onclick = function (){
										document.getElementById("pipeStartUploading").click();
									}
									$('#pipe-upload-wrap').keydown(function(event){
									  if (event.which == 13) {
									   event.preventDefault();
									   document.getElementById("pipeStartUploading").click();
									  }
									...
									document.getElementById("pipeStartUploading").onchange = function() {

										$.when(
										$.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/d_upload.js" ),
										$.Deferred(function( deferred ){
											$( deferred.resolve );
										 })
										).done(function(){
									...
									//API function call
									if ($.isFunction(window.onRecorderInit)){
									...
									$('#pipeStartRecording').keydown(function(event){
									  if (event.which == 13) {
									   event.preventDefault();
									   pipeInitRecorder();
									  }
									...
									//screen recording
									$('#pipeRecordScreen').hide();
									if(flashvars["srec"] && flashvars["srec"] != 0 && (accType == 50 || accType == 1) && webRtcClient == true){
										if((get_browser().name == "Chrome" && get_browser().version >= 72) || (get_browser().name == "Firefox" && get_browser().version >= 66)){
											$('#pipeRecordScreen').show();
											$('#pipeRecordScreen').click(pipeInitScreenRecorder);
											$('#pipeRecordScreen').keydown(function(event){
											  if (event.which == 13) {
											   event.preventDefault();
		...
		function pipeInitRecorder(){
			...
			if(webRtcClient == false){
				...
				$.when(
				    $.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/flash.js" ),
				    $.Deferred(function( deferred ){
					 $( deferred.resolve );
				    })
				).done(function(){});
				...
				$.when(
				    $.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/webrtc-js/adapter-latest.js" ),
				    $.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/webrtc-js/soundmeter.js" ),
				    $.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdnjs.cloudflare.com/ajax/libs/socket.io/2.1.1/socket.io.slim.js" ),
				    $.Deferred(function( deferred ){
					 $( deferred.resolve );
				    })
				).done(function(){
					$.getScript( ("https:" == document.location.protocol ? "https://" : "http://")+"cdn.addpipe.com/1.3/rtc.js" );
				});
